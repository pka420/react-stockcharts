...
import { getXCoordinate } from "../../coordinates/MouseCoordinateX";

class InteractiveXCoordinate extends Component {
    constructor(props) {
        super(props);
    ...
    drawOnCanvas(ctx, moreProps) {
        ...
        const xValue = edge.displayFormat(this.props.xValue);
        const yCoord = getXCoordinate(y1, xValue, newEdge, moreProps);
...
function helper(props, moreProps) {
    const { xValue, textBox} = props;

    const { chartConfig: { width, xScale, height } } = moreProps;

    const y = 10//Math.round(xScale(xValue));
...
InteractiveXCoordinate.propTypes = {
...
InteractiveXCoordinate.defaultProps = {
    onDragStart: noop,
    onDrag: noop,
    onDragComplete: noop,
...
export default InteractiveXCoordinate;
