...
import { InteractiveXCoordinate, DrawingObjectSelector } from "react-stockcharts/lib/interactive";
...
const alert = InteractiveXCoordinate.defaultProps.defaultPriceCoordinate;
const sell = {
	...InteractiveXCoordinate.defaultProps.defaultPriceCoordinate,
	stroke: "#E3342F",
	textFill: "#E3342F",
	text: "Sell 320",
	edge: {
		...InteractiveXCoordinate.defaultProps.defaultPriceCoordinate.edge,
		stroke: "#E3342F"
	}
};
const buy = {
...
class CandleStickChartWithInteractiveXCoordinate extends React.Component {
	constructor(props) {
		super(props);
		...
		if (this.state.enableInteractiveObject) {
			...
			if (independentCharts.length > 0) {
				...
				const {
					...
					chartConfig: { xScale, yScale },
				} = morePropsForChart;

				const yValue = round(yScale.invert(mouseY), 2);
				const xValue = round(xScale.invert(mouseX), 2);
				const newAlert = {
					...InteractiveXCoordinate.defaultProps.defaultPriceCoordinate,
					yValue,
                    xValue,
				...
				>
					...
					>
						...
						<InteractiveXCoordinate
							ref={this.saveInteractiveNodes("InteractiveXCoordinate", 1)}
					...
					<DrawingObjectSelector
						...
						drawingObjectMap={{
							InteractiveXCoordinate: "yCoordinateList"
						}}
...
CandleStickChartWithInteractiveXCoordinate.propTypes = {
...
CandleStickChartWithInteractiveXCoordinate.defaultProps = {
	type: "svg"
};

const CandleStickChart = fitWidth(
	CandleStickChartWithInteractiveXCoordinate
);
